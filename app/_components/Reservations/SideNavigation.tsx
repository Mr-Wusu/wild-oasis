      ...
      <ul className="flex flex-col gap-2 text-lg h-full font-josefineSans">
        {navLinks.map((link) => (
          <li key={link.name}>
            <Link
              href={link.href}
              className={`py-3 px-5 hover:bg-primary-4 transition-colors flex items-center gap-4 font-semibold text-primary-10 duration-300 focus:outline-none ${pathname === link.href ? "bg-primary-3":""}`}
            >
              {link.icon}
              {link.name}
            </Link>
